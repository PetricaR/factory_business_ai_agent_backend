...
class ADKSSEClient:
    ...
    def __init__(self, base_url: str, app_name: str = "agent_factory_ai",
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        app_name = st.text_input("App Name", value=os.getenv("APP_NAME", "agent_factory_ai"))
...
def main():
    ...
    if prompt:
        ...
        # Show a temporary "thinking" indicator (NOT a chat message)
        with st.spinner("🤔 Analyzing..."):
            st.session_state.streaming = True
            
            try:
                # Stream the response but don't display it
                # Just collect it
                response = ""
                for event in st.session_state.client.stream_query(prompt):
                    event_type = event.get("type")
                    
                    if event_type == "text_chunk":
                        response = event.get("full_text", "")
                    elif event_type == "final":
                        response = event.get("content", response)
                        break
                    elif event_type == "error":
                        response = f"❌ Error: {event.get('content', 'Unknown error')}"
                        break
