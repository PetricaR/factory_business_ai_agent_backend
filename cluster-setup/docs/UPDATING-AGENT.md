...
export IP=$(kubectl get service agent-factory-ai -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
curl http://$IP/health
...
2. **Push Phase** (~1 minute)
   ...
   ```
   [INFO] Pushing to GCR
   → Uploads to gcr.io/formare-ai/agent-factory-ai:20251120-010500
   → Also tags as :latest
   ```
...
kubectl rollout restart deployment agent-factory-ai
...
```bash
kubectl get pods
kubectl get deployment agent-factory-ai
```

### View Logs (See Your Updated Agent)

```bash
kubectl logs -l app=agent-factory-ai --tail=50 -f
```
...
```
gcr.io/formare-ai/agent-factory-ai:20251120-010500
gcr.io/formare-ai/agent-factory-ai:20251120-011230
gcr.io/formare-ai/agent-factory-ai:latest
```

**To rollback to specific version:**

```bash
kubectl set image deployment/agent-factory-ai \
  agent-factory-ai=gcr.io/formare-ai/agent-factory-ai:20251120-010500
```
...
kubectl logs -l app=agent-factory-ai --tail=20
...
kubectl get deployment agent-factory-ai -o yaml | grep image:
```
...
# Verify rollback
kubectl rollout status deployment agent-factory-ai
```
...
kubectl logs -l app=agent-factory-ai -f  # 5. Verify
```
