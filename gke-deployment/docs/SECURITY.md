...
kind: BackendConfig
metadata:
  name: agent-factory-ai-backend-config
spec:
  iap:
    enabled: true
...
kind: Service
metadata:
  name: agent-factory-ai
  annotations:
    cloud.google.com/backend-config: '{"default": "agent-factory-ai-backend-config"}'
spec:
...
kind: Ingress
metadata:
  name: agent-factory-ai-ingress
  annotations:
    kubernetes.io/ingress.class: "gce"
    kubernetes.io/ingress.global-static-ip-name: "agent-factory-ai-ip"
spec:
  defaultBackend:
    service:
      name: agent-factory-ai
...
IAP_SERVICE=$(gcloud compute backend-services list --format="value(name)" --filter="name~agent-factory-ai")
...
kubectl get ingress agent-factory-ai-ingress
