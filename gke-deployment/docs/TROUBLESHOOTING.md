...
gcloud projects get-iam-policy PROJECT_ID \
  ...
  --filter="bindings.members:serviceAccount:agent-factory-ai-sa@PROJECT_ID.iam.gserviceaccount.com"
...
2. **Check Events:**
   ...
   ```bash
   kubectl describe service agent-factory-ai | grep Events -A 10
   ```
...
```bash
# Get external IP
EXTERNAL_IP=$(kubectl get service agent-factory-ai -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
...
3. **Service misconfiguration:**
   ...
   kubectl get service agent-factory-ai -o yaml | grep selector -A 2
   kubectl get pods -l app=agent-factory-ai --show-labels
   ```
...
```
error: deployment "agent-factory-ai" exceeded its progress deadline
```
...
kubectl describe deployment agent-factory-ai >> diagnosis.txt
kubectl logs -l app=agent-factory-ai --tail=500 >> diagnosis.txt
